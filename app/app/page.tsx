...
import { Plus, Trash2, SplitSquareVertical, Edit2, ChevronRight, ChevronDown, ChevronUp, Loader2, Copy, Check } from "lucide-react"
...
export default function Component() {
  ...
  const [editingTask, setEditingTask] = useState<{ _id: string, parentId?: string } | null>(null)
  const [editText, setEditText] = useState("")
  const [isJsonOpen, setIsJsonOpen] = useState(false)
  const [isCopied, setIsCopied] = useState(false)

  const [tasks, setTasks] = useState<Task[]>([
    { _id: "1", text: "Design new website layout", completed: false, subtasks: [], collapsed: false },
    { _id: "2", text: "Implement user authentication", completed: false, subtasks: [], collapsed: false },
    { _id: "3", text: "Create content management system", completed: false, subtasks: [], collapsed: false },
  ]);
  ...
  const addGeneratedSubtasks = () => {
    if (breakdownTask) {
      const newSubtasks = generateSubtasks(breakdownTask.text, subtaskCount).map(subtask => ({
      ...
      setTasks(tasks.map(task => 
        task._id === breakdownTask._id ? { ...task, subtasks: [...task.subtasks, ...newSubtasks].slice(0, 5) } : task
      ))
        ...
        if (task._id === (editingTask.parentId || editingTask._id)) {
          if (editingTask.parentId) {
            return {
              ...task,
              subtasks: task.subtasks.map(subtask =>
                subtask._id === editingTask._id ? { ...subtask, text: editText } : subtask
              )
            }
          }
  ...
  const renderTask = (task: Task, parentId?: string) => (
    <li key={task._id} className={`mb-2 ${parentId !== undefined ? 'ml-6 border-l-2 border-gray-300 pl-4' : ''}`}>
      <div className="flex items-center justify-between bg-gray-100 p-2 rounded">
        <div className="flex items-center flex-grow">
          ...
          <Checkbox
            ...
            onCheckedChange={() => toggleTask(parentId || task._id, parentId ? task._id : undefined)}
            id={`task-${task._id}`}
          />
          {editingTask?._id === task._id ? (
            ...
            <Label
              ...
              className={`ml-2 flex-grow ${task.completed ? 'line-through text-gray-500' : ''}`}
            >
              {task.text}
            </Label>
          )}
          ...
          {editingTask?._id === task._id ? (
            ...
            <>
              ...
              <Button
                variant="ghost"
                size="icon"
                onClick={() => removeTask(parentId || task._id, parentId ? task._id : undefined)}
                title="Remove task"
              >
                <Trash2 className="h-4 w-4" />
              </Button>
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
        <h1 className="text-3xl font-bold mb-6">5ify - Anything in less than 5 steps.</h1>
        ...
        <Dialog open={breakdownTask !== null} onOpenChange={() => setBreakdownTask(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Break down task: {breakdownTask?.text}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <Label>Number of subtasks to generate:</Label>
              <Slider
                min={1}
